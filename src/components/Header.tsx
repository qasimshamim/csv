            ...
            <ul className="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0">
              {
                navLinks.length > 0 && navLinks.map((v, i) => (
                  <li key={i}  className={`${v.link == pathname ? 'text-TLightBlue font-bold' : 'nav text-white'}  cursor-pointer capitalize duration-300 transition-all relative before:h-1 before:bg-Tblue before:absolute before:-bottom-1 before:duration-300 before:w-0 before:transition-all before:left-0 before:rounded-full hover:before:w-full hover:text-Tblue group`}>
                   <span className='w-full h-full' onClick={() => router.push(v.link)}> {v.title}</span>
                    {v.menu && <DropDown />}
                  </li>
                ))
              }
